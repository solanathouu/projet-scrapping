import urllib.request #Faire des requêtes HTTP
import json #Manipuler le format JSON
import csv #mettre des outputs en csv
...
# penser a remplacer le cookies tout les jours
headers = {
    "Content-Type": "application/json",  #Le serveur doit savoir quel format vous utilisez
    "Cookie": cookie,                    #Pour prouver que vous êtes connecté
    "Accept-Language": "fr-FR",          #Le serveur refuse si vous n'indiquez pas de langue
    "Site": "FR",                        #TD SYNNEX a plusieurs pays (FR, US, UK...)
    "Consumer": "INTOUCH"                #Indique quelle partie du site vous utilisez
}
#headers sont des infos demandées par le site qui nous permettent de communiquer avec lui 

URL_SEARCH = "https://intouch.tdsynnex.com/ui/ui-intouch-search/v1/public/fullsearch" #pour aller chercher les données
URL_PRICE = "https://intouch.tdsynnex.com/ui/ui-intouch-search/v3/public/price" #pour aller chercher les prix
#2 requetes car prix et caracteristiques ne sont pas dans le meme url

CATEGORY_ID = "FCS" #on touche pas
CATEGORY_VALUE_ID = "FCS_SUBCLASS_PCALLIN1_COMDESK" #a changer pour chaque categories
PAGE_SIZE = 50 #produit recup par pages
SORT_TYPE = "STOCK" #tri par stock on touche pas
OUTPUT_CSV = "produits_allinone.csv" #output en csv pour mettre sur sheet // changer le nom pour chaque categorie

def post_json(url, payload): #pour communiquer avec le site et leur demander les données
    req = urllib.request.Request(url, data=json.dumps(payload).encode(), headers=headers)

# urllib.request.Request(...)  
# Crée la requête HTTP avec l'URL, les données et les headers

# .encode()  
# Convertit le texte en bytes (langage machine)
# Nécessaire pour envoyer via internet

# json.dumps(payload)  
# Convertit un dictionnaire Python en texte JSON
# Exemple : {"page": 1} → '{"page": 1}'

    return json.loads(urllib.request.urlopen(req).read()) 

# urllib.request.urlopen(req)  # Envoie la requête
# .read()                      # Lit la réponse (en bytes)
# json.loads(...)              # Convertit la réponse JSON en dictionnaire Python


tous_produits, page = [], 1  #création de la liste produit
while True: #boucle infine
    data = {
        ...
        "refinementGroups": [{
            "group": "Categories",
            "refinements": [{"id": CATEGORY_ID, "valueId": CATEGORY_VALUE_ID}] #chercher manuelement le produit
        }]
    }
...
# version longue: créer une liste avec les ids
# ids = []
# for p in tous_produits:
#     ids.append(p["id"])

prix_data = post_json(URL_PRICE, {"ids": ids}) # requete de prix par id
